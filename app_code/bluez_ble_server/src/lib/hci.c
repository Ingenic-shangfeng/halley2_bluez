...
static bdaddr_t random_address = {{0}};
typedef struct {
	char *str;
	unsigned int val;
} hci_map;
			...
			long arg)
{
	...
	dl = mem_malloc(HCI_MAX_DEV * sizeof(*dr) + sizeof(*dl));
	if (!dl) {
		err = errno;
		goto done;
	}
		...
		inquiry_info **ii, long flags)
{
	...
	buf = mem_malloc(sizeof(*ir) + (sizeof(inquiry_info) * (nrsp)));
	if (!buf)
		goto done;
...
int hci_le_set_advertise_parameters(int dd,
...
int hci_le_set_random_address(int dd, const bdaddr_t *bdaddr, int to)
{
	struct hci_request rq;
	...
	bacpy(&random_address, bdaddr);
	return 0;
}
int hci_le_get_random_address(bdaddr_t *bdaddr)
{
	char ba[18];
	*bdaddr = random_address;
	ba2str(bdaddr, ba);
